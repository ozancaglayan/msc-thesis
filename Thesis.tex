...
\par{
    P300 is a widely used and researched ERP in BCI field. It is a positive ERP component (a positive
    amplitude change happens in the ongoing activity) elicited by the brain approximately
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[scale=0.6]{images/p300_speller}
        \caption{Layout of the Farwell-Donchin P300 Speller}
        \label{fig:p300_speller}
    \end{figure}
}
\par{
    P300 response is also evoked in auditory and tactile stimulation paradigms.
}
...
\par{
    ...
    the current going to the light sources which can be powered by an external power source. The final
    montage of the stimulator using LED matrices can be seen in Figure ~\ref{fig:bbb_ssvep}.
    \begin{figure}[ht]
    \centering
...
\par{
    The correctness of the flickering frequencies is tested using an Arduino and a simple oscilloscope
    ...
    It has been observed that the flickering frequencies are pretty accurate during a BCI experiment, e.g. a drift of maximum 0.04Hz is measured
    for a recording session with stimulation frequencies tuned to $f_1=17Hz$ and $f_2=19Hz$. This led us to the decision
    that using the PRU for SSVEP stimulation will sophisticate the design.\\\\\\
}
...
\par{
    We currently tuned the SSVEP stimulator to flicker two RED 5x7 LED matrices at
    frequencies $f_1=17Hz$ and $f_2=19Hz$ to represent the left and the right
    arm of the humanoid robot. We only enabled 9 LEDs per LED matrix as turning on
    all of them is exhausting for visual perception.
}
\par{
