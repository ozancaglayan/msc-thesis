...
\par{
    ...
    which runs BCI software written purely in Python on Linux OS. We believe that
...
\mysubsubsection{Python-emotiv}\label{seq:emotiveeg_python_emotiv}
\par{
...
\mysubsection{SSVEP Stimulation}\label{seq:ssvep_stimulation}
\mysubsubsection{SSVEP Stimulation on BBB}\label{seq:ssvep_stimulation_hw}
\par{
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.6\textwidth]{images/led_circuit}
    ...
    Input/Output capabilities for LED SSVEP stimulation.
    ...
    For this study, a simple digital circuit is designed to drive the LEDs using transistors (Figure ~\ref{fig:bbb_led_schema}).
    The use of transistors is important because BBB can not provide enough current
    through its GPIO pins to light an LED brightly. The transistors rapidly switch
    the LED circuits which can be powered by an external power source.
}
\mysubsubsection{Stimulation Software}\label{seq:ssvep_stimulation_sw}
\par{
    A Python script is written to manage the external stimulation circuit. We do not
...
\mysubsection{Experiment Workflow}\label{seq:bci_details}
\par{
}

\mysubsection{Building Blocks}\label{seq:bci_workflow}
\par{
    \begin{figure}[ht]
        \centering
        \includegraphics[scale=0.75]{images/workflow}
        \caption{BCI Workflow.}
        \label{fig:workflow}
    \end{figure}

    Our BCI design is splitted into 3 separate Python programs
