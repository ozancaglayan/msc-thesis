...
\par{
    ...
    Note that only 9 LEDs per LED matrix are enabled as using all of them is exhausting for visual perception.
    \begin{figure}[ht]
    \centering
...
\par{
    ...
    %    \label{fig:workflow}
    %\end{figure}
    The EEG acquisition (ACQ) block is actually what manages the SSVEP and the SPC block.
...
\par{
    ...
    it signals the SSVEP block to start the flickering.
    ACQ continuously acquires 2 seconds length EEG chunks
    and passes them to the SPC block until SPC sends back a classification
    result. Once the result is received, ACQ signals the SSVEP block to stop the
    flickering and sends a control command to the robot according to the result.
}
\par{
    The proposed BCI can run infinitely until the user or someone
